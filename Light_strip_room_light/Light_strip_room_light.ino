...
String light_str; //see last code block below use these to convert the float that you get back from DHT to a string =str
char light_chr[50];

/************ WIFI and MQTT Information (CHANGE THESE FOR YOUR SETUP) ******************/
const char* ssid = "WhTrUnifi24"; //type your WIFI information inside the quotes
const char* password = "basH3Rfr";
const char* mqtt_server = "10.10.0.22";
const char* mqtt_username = "";
const char* mqtt_password = "";
const int mqtt_port = 1883;

/**************************** FOR OTA **************************************************/
#define SENSORNAME "Light_Strip_Upper_ESP" //change this to whatever you want to call your device
#define OTApassword "5533" //the password you will need to enter to upload remotely via the ArduinoIDE
int OTAport = 8266;


/************* MQTT TOPICS (change these topics as you wish)  **************************/
const char* light_state_topic = "whit33r/stripUp";
const char* light_set_topic = "whit33r/stripUp/set";
...
void setup() {
  ...
  client.setCallback(callback);
    WiFi.hostname("Light_Strip_Room_ESP");
  //OTA SETUP
...
int calculateVal(int step, int val, int i) {
  if ((step) && i % step == 0) { // If step is non-zero and its time to change a value,
    ...
    else if (step < 0) {         //   ...or decrement it if step is
      val -= 1;
    }
  }
...
void timedTasks() {
  if ((millis() > TTasks + (kUpdFreq*10000)) || (millis() < TTasks)) { 
    TTasks = millis();
    lightSensor();
        Serial.println("task lefut");
  }
...
light_str = String(photocellReading); //converting ftemp (the float variable above) to a string 
    light_str.toCharArray(light_chr, light_str.length() + 1); //packaging up the data to publish to mqtt whoa...
